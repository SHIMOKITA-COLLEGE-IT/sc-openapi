...
mod paths {
    ...
    pub(crate) static ID_USERS: usize = 5;
    pub(crate) static ID_USERS_SEARCH: usize = 6;
    pub(crate) static ID_USERS_RECORDID: usize = 7;
    lazy_static! {
        ...
        match method {
            ...
            hyper::Method::GET if path.matched(paths::ID_GENERATIONS_RECORDID) => {
                ...
                let param_record_id = match percent_encoding::percent_decode(path_params["recordId"].as_bytes()).decode_utf8() {
                    Ok(param_record_id) => match param_record_id.parse::<serde_json::Value>() {
            ...
            hyper::Method::POST if path.matched(paths::ID_LOGIN) => {
                ...
                match result {
                            Ok(body) => {
                                ...
                                let param_post_login_request: Option<models::PostLoginRequest> = if !body.is_empty() {
                                    let deserializer = &mut serde_json::Deserializer::from_slice(&*body);
                                    match serde_ignored::deserialize(deserializer, |path| {
                                            warn!("Ignoring unknown field in body: {}", path);
                                            unused_elements.push(path.to_string());
                                    }) {
                                        Ok(param_post_login_request) => param_post_login_request,
                                        Err(_) => None,
                                    }
                                } else {
                                    None
                            ...
                            Err(e) => Ok(Response::builder()
                                                .status(StatusCode::BAD_REQUEST)
                                                .body(Body::from(format!("Couldn't read body parameter PostLoginRequest: {}", e)))
                                                .expect("Unable to create Bad Request response due to unable to read body parameter PostLoginRequest")),
                        }
            },
            ...
            hyper::Method::PUT if path.matched(paths::ID_USERS) => {
                ...
                match result {
                            Ok(body) => {
                                let mut unused_elements = Vec::new();
                                let param_put_users_request: Option<models::PutUsersRequest> = if !body.is_empty() {
                                    let deserializer = &mut serde_json::Deserializer::from_slice(&*body);
                                    match serde_ignored::deserialize(deserializer, |path| {
                                            warn!("Ignoring unknown field in body: {}", path);
                                            unused_elements.push(path.to_string());
                                    }) {
                                        Ok(param_put_users_request) => param_put_users_request,
                                        Err(_) => None,
                                    }
                                } else {
                                    None
                            ...
                            Err(e) => Ok(Response::builder()
                                                .status(StatusCode::BAD_REQUEST)
                                                .body(Body::from(format!("Couldn't read body parameter PutUsersRequest: {}", e)))
                                                .expect("Unable to create Bad Request response due to unable to read body parameter PutUsersRequest")),
                        }
            },
