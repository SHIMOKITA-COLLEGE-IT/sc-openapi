    ...
    C: Has<XSpanIdString> + Has<Option<AuthData>> + Clone + Send + Sync + 'static,
{
    ...
    async fn get_generations_record_id(
        &self,
        param_record_id: serde_json::Value,
    ...
    async fn get_groups(
        &self,
        param_r#type: Option<serde_json::Value>,
    ...
    async fn post_login(
        &self,
        param_post_login_request: Option<models::PostLoginRequest>,
        context: &C) -> Result<PostLoginResponse, ApiError>
    {
        ...
        let body = param_post_login_request.map(|ref body| {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::PostLogin200Response>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
    ...
    async fn put_users(
        &self,
        param_put_users_request: Option<models::PutUsersRequest>,
        context: &C) -> Result<PutUsersResponse, ApiError>
    {
        ...
        let body = param_put_users_request.map(|ref body| {
            serde_json::to_string(body).expect("impossible to fail to serialize")
        });
